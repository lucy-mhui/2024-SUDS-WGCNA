---
title: "2 automatic network construction and module detection"
output: 
  html_document:
    message: FALSE
date: "`r Sys.Date()`"
...
for (power in 1:20) {
  ...
  net <- blockwiseModules(MDDinfData5, power = power,
                           TOMType = "unsigned", minModuleSize = 1,
                           reassignThreshold = 0, mergeCutHeight = 0.25,
                           numericLabels = TRUE, pamRespectsDendro = FALSE,
                           saveTOMs = TRUE,
...
softPower=10
adjacency=adjacency(MDDinfData5,power=softPower)
...
MEList = moduleEigengenes(MDDinfData5,colors=dynamicColors)
...
# choose height cut
MEDISSThres=0.25
merge=mergeCloseModules(MDDinfData5,dynamicColors,cutHeight=MEDISSThres,verbose=3)
...
MEs=merge$newMEs
save(MEs, moduleLabels,moduleColors,geneTree,file="02-MDD-networkConstruction.RData")
```
...
# call network topology analysis function
sft = pickSoftThreshold(CntrlinfData5, powerVector = powers, verbose = 5)
...
```{r}
# automatic construction is not working -- see below for step by step construction
net = blockwiseModules(CntrlinfData5, power = 6,
                       TOMType = "unsigned", minModuleSize = 1,
                       reassignThreshold = 0, mergeCutHeight = 0.25,
                       numericLabels = TRUE, pamRespectsDendro = FALSE,
                       saveTOMs = TRUE,
...
make a table of various soft thresholding outcomes
...
for (power in 1:20) {
  # Run blockwiseModules for the current power
  net <- blockwiseModules(CntrlinfData5, power = power,
                           TOMType = "unsigned", minModuleSize = 1,
                           reassignThreshold = 0, mergeCutHeight = 0.25,
                           numericLabels = TRUE, pamRespectsDendro = FALSE,
                           saveTOMs = TRUE,
...
# calculate adjacencies
softPower=6
adjacency=adjacency(CntrlinfData5,power=softPower)
...
moduleLabels=match(moduleColors,colorOrder)-1
MEs=merge$newMEs
save(MEs, moduleLabels,moduleColors,geneTree,file="02-Control-networkConstruction.RData")
```
